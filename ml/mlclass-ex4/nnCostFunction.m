...
%(lambda/(2*m))*(sum(sum(Theta1_new .* Theta1_new))+(sum(sum(Theta2_new .* Theta2_new))))


%(lambda/(2*m))*(sum(sum(Theta1(2:end,:).^2))+(sum(sum(Theta2(2:end,:).^2))))

% J = (1/m)*(sum(sum((-y.*log_h)-((1-y).*(log_minus_h)))))
J = (1/m)*(sum(sum((-y.*log_h)-((1-y).*(log_minus_h))))) + (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)))
% -------------------------------------------------------------
